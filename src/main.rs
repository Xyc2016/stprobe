...
fn inspect_input(input: &str) -> Result<Report, InspectError> {
...
fn inspect_remote_file(input: &str, url: &Url) -> Result<Report, InspectError> {
    ...
    let file_size = fetch_file_size_and_validate_ranges(&client, url)?;
    let header_len = fetch_header_length(&client, url)?;
...
fn fetch_file_size_and_validate_ranges(client: &Client, url: &Url) -> Result<u64, InspectError> {
    let response = ranged_get(client, url, 0, HEADER_PREFIX_LEN - 1)?;
    if response.status() != StatusCode::PARTIAL_CONTENT {
        return Err(InspectError::RangeUnsupported(url.to_string()));
    }

    parse_total_size(&response, url)
}

fn fetch_header_length(client: &Client, url: &Url) -> Result<usize, InspectError> {
    let response = ranged_get(client, url, 0, HEADER_PREFIX_LEN - 1)?;
    if response.status() != StatusCode::PARTIAL_CONTENT {
        return Err(InspectError::RangeUnsupported(url.to_string()));
    }
...
fn parse_total_size(response: &Response, url: &Url) -> Result<u64, InspectError> {
    ...
    let total =
        content_range
            ...
            .ok_or_else(|| InspectError::InvalidRemoteResponse {
                url: url.to_string(),
                reason: "malformed Content-Range header".to_owned(),
            })?;
    ...
    total
        ...
        .map_err(|_| InspectError::InvalidRemoteResponse {
            url: url.to_string(),
            reason: "invalid total size in Content-Range header".to_owned(),
        })
}
